...
public class EgrangKeybindUI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        foreach (KeyCode key in System.Enum.GetValues(typeof(KeyCode)))
        {
            ...
            {
                KeybindManager.Instance.egrangPlayer1Left = key;
                waitingForPlayer1Left = false;
            }
            else if (waitingForPlayer1Right)
            {
                KeybindManager.Instance.egrangPlayer1Right = key;
                waitingForPlayer1Right = false;
            }
            else if (waitingForPlayer2Left)
            {
                KeybindManager.Instance.egrangPlayer2Left = key;
                waitingForPlayer2Left = false;
            }
            else if (waitingForPlayer2Right)
            {
                KeybindManager.Instance.egrangPlayer2Right = key;
    ...
    public void RefreshUI()
    {
        ...
        SetText(player1LeftText, KeybindManager.Instance.egrangPlayer1Left.ToString());
        SetText(player1RightText, KeybindManager.Instance.egrangPlayer1Right.ToString());
        SetText(player2LeftText, FormatKey(KeybindManager.Instance.egrangPlayer2Left));
        SetText(player2RightText, FormatKey(KeybindManager.Instance.egrangPlayer2Right));
    }
