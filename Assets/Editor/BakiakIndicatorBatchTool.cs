...
public class BakiakIndicatorBatchTool : EditorWindow
{
    ...
    float player1VerticalMargin = 0.12f;
    ...
    float player2VerticalMargin = 0.12f;
    ...
    [MenuItem("Tools/Nusantara Battle/Update Bakiak Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<BakiakIndicatorBatchTool>("Bakiak Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Bakiak Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Bakiak, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Bakiak Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Bakiak Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Bakiak Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Bakiak scene(s).",
            "OK");
    }
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Bakiak Indicator Settings");
