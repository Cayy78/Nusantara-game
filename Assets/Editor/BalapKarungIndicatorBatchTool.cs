...
public class BalapKarungIndicatorBatchTool : EditorWindow
{
    ...
    [MenuItem("Tools/Nusantara Battle/Update Balap Karung Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<BalapKarungIndicatorBatchTool>("Balap Karung Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Balap Karung Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Balap Karung, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Balap Karung Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Balap Karung Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Balap Karung Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Balap Karung scene(s).",
            "OK");
    }
    ...
    IEnumerable<string> EnumerateScenePaths()
    {
        ...
        foreach (string sceneGuid in sceneGuids)
        {
            ...
            if (!scenePath.Contains("Balap Karung"))
                continue;
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Balap Karung Indicator Settings");
