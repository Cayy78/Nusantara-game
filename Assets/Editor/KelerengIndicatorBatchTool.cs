...
public class KelerengIndicatorBatchTool : EditorWindow
{
    ...
    [MenuItem("Tools/Nusantara Battle/Update Kelereng Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<KelerengIndicatorBatchTool>("Kelereng Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Kelereng Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Kelereng, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Kelereng Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Kelereng Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Kelereng Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Kelereng scene(s).",
            "OK");
    }
    ...
    IEnumerable<string> EnumerateScenePaths()
    {
        ...
        foreach (string sceneGuid in sceneGuids)
        {
            ...
            if (!scenePath.Contains("Kelereng"))
                continue;
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Kelereng Indicator Settings");
