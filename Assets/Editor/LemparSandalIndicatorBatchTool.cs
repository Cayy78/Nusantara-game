...
public class LemparSandalIndicatorBatchTool : EditorWindow
{
    const string SceneRootPath = "Assets/Scenes/Scene Gameplay";

    Color player1Color = new Color32(255, 0, 0, 255);
    float player1HorizontalMargin = -0.15f;
    float player1VerticalMargin = -0.62f;
    ...
    float player2HorizontalMargin = -0.15f;
    float player2VerticalMargin = -0.62f;
    ...
    [MenuItem("Tools/Nusantara Battle/Update Lempar Sandal Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<LemparSandalIndicatorBatchTool>("Lempar Sandal Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Lempar Sandal Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Lempar Sandal, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Lempar Sandal Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Lempar Sandal Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Lempar Sandal Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Lempar Sandal scene(s).",
            "OK");
    }
    ...
    IEnumerable<string> EnumerateScenePaths()
    {
        ...
        foreach (string sceneGuid in sceneGuids)
        {
            ...
            if (!scenePath.Contains("Lempar Sandal"))
                continue;
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Lempar Sandal Indicator Settings");
