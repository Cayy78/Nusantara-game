...
public class LompatTaliIndicatorBatchTool : EditorWindow
{
    const string SceneRootPath = "Assets/Scenes/Scene Gameplay";

    Color player1Color = new Color32(255, 0, 0, 255);
    float player1HorizontalMargin = -0.11f;
    float player1VerticalMargin = -0.31f;
    ...
    float player2HorizontalMargin = -0.11f;
    float player2VerticalMargin = -0.31f;
    ...
    [MenuItem("Tools/Nusantara Battle/Update Lompat Tali Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<LompatTaliIndicatorBatchTool>("Lompat Tali Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Lompat Tali Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Lompat Tali, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Lompat Tali Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Lompat Tali Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Lompat Tali Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Lompat Tali scene(s).",
            "OK");
    }
    ...
    IEnumerable<string> EnumerateScenePaths()
    {
        ...
        foreach (string sceneGuid in sceneGuids)
        {
            ...
            if (!scenePath.Contains("Lompat Tali"))
                continue;
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Lompat Tali Indicator Settings");
