...
public class PakuBotolIndicatorBatchTool : EditorWindow
{
    const string SceneRootPath = "Assets/Scenes/Scene Gameplay";

    Color player1Color = new Color32(255, 0, 0, 255);
    float player1HorizontalMargin = -0.33f;
    float player1VerticalMargin = -0.68f;
    ...
    float player2HorizontalMargin = -0.32f;
    float player2VerticalMargin = -0.68f;
    ...
    [MenuItem("Tools/Nusantara Battle/Update Paku Botol Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<PakuBotolIndicatorBatchTool>("Paku Botol Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Paku Botol Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Paku Botol, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Paku Botol Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Paku Botol Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Paku Botol Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Paku Botol scene(s).",
            "OK");
    }
    ...
    IEnumerable<string> EnumerateScenePaths()
    {
        ...
        foreach (string sceneGuid in sceneGuids)
        {
            ...
            if (!scenePath.Contains("Paku Botol"))
                continue;
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Paku Botol Indicator Settings");
