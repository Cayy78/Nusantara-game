...
public class TarikTambangIndicatorBatchTool : EditorWindow
{
    ...
    [MenuItem("Tools/Nusantara Battle/Update Tarik Tambang Indicator Settings")]
    static void OpenWindow()
    {
        GetWindow<TarikTambangIndicatorBatchTool>("Tarik Tambang Indicators");
    }

    void OnGUI()
    {
        EditorGUILayout.LabelField("Tarik Tambang Indicator Batch Tool", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Tool ini hanya mengubah setting panah untuk scene Tarik Tambang, baik single player maupun multiplayer.",
            MessageType.Info);

        EditorGUILayout.Space();

        if (GUILayout.Button("Scan Tarik Tambang Scenes"))
            ScanScenes();

        if (GUILayout.Button("Apply To Tarik Tambang Scenes"))
            ApplyToScenes();
    ...
    void ApplyToScenes()
    {
        ...
        EditorUtility.DisplayDialog(
            "Tarik Tambang Indicator Update Complete",
            "Updated " + updatedComponentCount + " loader component(s) across " + changedSceneCount + " Tarik Tambang scene(s).",
            "OK");
    }
    ...
    IEnumerable<string> EnumerateScenePaths()
    {
        ...
        foreach (string sceneGuid in sceneGuids)
        {
            ...
            if (!scenePath.Contains("Tarik Tambang"))
                continue;
    ...
    bool ApplySettings(MonoBehaviour loader)
    {
        ...
        bool changed = false;
        Undo.RecordObject(loader, "Update Tarik Tambang Indicator Settings");
